...
func TestCallingFunctionsWithWrongArguments(t *testing.T) {
    ...
    for _, tt := range tests {
        ...
        err = vm.Run()
        if err == nil {
            t.Fatalf("VM error expected but no error received.")
        }

        if err.Error() != tt.expected {
            t.Fatalf("wrong VM error: got=%q, want=%q", err, tt.expected)
        }
    }
}
...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case map[object.HashKey]int64:
		...
		for expectedKey, exexpectedValue := range expected {
			...
			if !ok {
				t.Errorf("not pair for given key in Pairs")
			}

			err := testIntegerObject(exexpectedValue, pair.Value)
			if err != nil {
				t.Errorf("testIntegerObject failed: %s", err)
			}
		}
